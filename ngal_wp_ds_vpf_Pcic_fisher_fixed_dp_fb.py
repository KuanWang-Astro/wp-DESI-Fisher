...
parser.add_argument('--ptclrate',type=float,default=0.1,dest='ptclrate')
...
output_names = ('func_all','param','i')  ##ngal(1)+wp(29)+ggl(30)+vpf(30)+Pcic(70)
...
if args.vpfcen:
    ...
    vpf_centers = np.random.rand(num_sphere,3)*Lbox
##vpf
...
rp_bins_ggl = np.logspace(-1, 1.5, 31)
##ggl
...
def calc_all_observables(param):
    ...
    func = np.concatenate((np.array((pos_gals_d.shape[0]/float(Lbox**3),)), wprp, ggl, vpf, Pcic))
    ...
    with Pool(nproc) as pool:
        if 1:
            ...
            if args.ptclpos:
                ...
                ptclpos = return_xyz_formatted_array(*(table[ax] for ax in 'xyz'), period=Lbox)
