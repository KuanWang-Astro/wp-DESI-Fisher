...
output_names = ('func_all','param','i')  ##ngal(1)+wp(19)+vpf(15)+Pcic(70)+ggl(20)
...
def calc_all_observables(param):
    ...
    func = np.concatenate((np.array((pos_gals_d.shape[0]/float(Lbox**3),)), wprp, vpf, Pcic, ggl))
...
def main(model_gen_func, params_fname, output_fname):
    global model
    model = model_gen_func()
    global median_w
    median_w = np.median(np.loadtxt(params_fname, usecols=range(7)),axis=0)
    params = median_w*np.ones((7*args.Nparam,7))  ##take medians
    
    dp_range = np.array((0.1,0.05,0.02,0.1,0.02,0.05,0.05))

    for i in range(7):
        params[args.Nparam*i:args.Nparam*(i+1),i] += (2.*np.random.random(args.Nparam)-1)*dp_range[i]
...
if __name__ == '__main__':
    main(decorated_hod_model, args.infile, args.outfile)
